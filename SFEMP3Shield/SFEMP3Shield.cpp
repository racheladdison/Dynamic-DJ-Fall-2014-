...
//—
/**
 * \brief Get Track’s Year
 *
 * \param[out] info buffer pointer char array to be updated with result
 *
 * Extract the Year from the current file handles track ID3 tag information.
 *
 * \warning ID3 Tag information may not be present on all source files.
 * Otherwise may result in non-sense.
 * It is possible to add it with common tools outside of this project.
 */
void SFEMP3Shield::trackYear(char* infobuffer){
  getTrackInfo(TRACK_YEAR, infobuffer);
}

//————
/**
 * \brief Get Track’s Genre
 *
 * \param[out] info buffer pointer char array to be updated with result
 *
 * Extract the Album from the current file handles track ID3 tag information.
 *
 * \warning ID3 Tag information may not be present on all source files.
 * Otherwise may result in non-sense.
 * It is possible to add it with common tools outside of this project.
 *\
void SFEMP3Shield::trackGenre(int* infobuffer){
  getTrackGenre(TRACK_GENRE, infobuffer);
}

void SFEMP3Shield::getTrackGenre(uint8_offset, int* infobuffer){
  ...
  int numb=0;
  for(x=0; x<9; x++){
    num=num+(bitread(-128+offset, x)<<x);
  }
...
void SFEMP3Shield::getTrackInfo(uint8_t offset, char* infobuffer){
  ...
  if((offset==TRACK_ALBUM)||(offset==TRACK_ARTIST)||(offset==TRACK_TITLE)){
    //read 30 bytes of tag informat at -128 + offset
    track.read(infobuffer, 30);
    infobuffer = strip_nonalpha_inplace(infobuffer);
  }

  if((offset==TRACK_YEAR)){
    //read 4 bytes of tag informat at -128 + offset
    track.read(infobuffer, 4);
    infobuffer = strip_nonalpha_inplace(infobuffer);
  }
